...
def load_user_settings():
    """Încarcă setările utilizatorului din JSON"""
    if not os.path.exists(USER_SETTINGS_PATH):
        print(f"⚠️ user_settings.json nu a fost găsit la {USER_SETTINGS_PATH}")
        return {}
    try:
        with open(USER_SETTINGS_PATH, "r", encoding="utf-8") as f:
            settings = json.load(f)
        print(f"✔ user_settings.json încărcat cu succes: {settings}")
        return settings
    except Exception as e:
        print(f"⚠️ Eroare la citirea user_settings.json: {e}")
        return {}
...
class ModernPDFInvoice(FPDF):
    ...
    def header(self):
        ...
        self.add_font("DejaVu", "BI", FONT_PATH)
...
def generate_invoice_pdf(row, user_settings=None):
    ...
    print(f"Generating PDF with settings: company={company}, seller={seller}")
    ...
    seller_content = [
        company.get('name', 'Nu este setat'),
        f"CUI: {company.get('cui', 'Nu este setat')}",
        f"ID Legal: {seller.get('legal_id', 'Nu este setat')}",
        f"ID TVA: {seller.get('vat', 'Nu este setat')}",
        f"Strada: {seller.get('street', 'Nu este setat')}",
        f"Oraș: {seller.get('city', 'Nu este setat')}",
        f"Județ: {seller.get('county', 'Nu este setat')}",
        f"Țară: {seller.get('country', 'Nu este setat')}"
    ]
    create_info_card(pdf, 15, 75, 85, 40, "VÂNZĂTOR", seller_content, pdf.WHITE)

    buyer_content = [
        safe_get(row, 'Nume cumpărător'),
        f"ID Legal: {safe_get(row, 'ID legal cumpărător')}",
        f"ID TVA: {safe_get(row, 'ID TVA cumpărător')}",
        safe_get(row, 'Stradă cumpărător'),
...
def generate_all_invoices(df, user_settings=None):
    ...
    for index, row in df.iterrows():
        try:
            ...
            print(f"✓ Factura {row.get('Număr factură')} generată: {file_path}")
        except Exception as e:
            print(f"✗ Eroare la generarea facturii {row.get('Număr factură')}: {e}")
